...
## クライアント(pc/ tablet/ mobile/)分離→再統合の経緯(2026-09-10〜2026-09-15)

以下は2026-09-10に`aon-co-jp/open-english-pc`へsubmodule切り出しした際の
移設チェックリスト(そのまま保存)。2026-09-15、分離の目的
（クライアントとサーバー/デモ環境の分離）が実現されないまま二重管理の
コストだけが残っていたため、履歴を保持して本体へ再統合し、
`open-english-pc`リポジトリは削除した。以下は**歴史的記録**として残す。
