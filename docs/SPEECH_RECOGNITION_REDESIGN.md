...
> ステータス: **P0 設計確定(2026-08-29 起票)。次は P1-α のプロトタイプ**
...
| フェーズ | プロトタイプ | 受け入れ基準(前周比) |
|---|---|---|
| **P1-α** | §4.1 A のみ(BCP-47 言語修正) | 非英日言語の WER が明確に低下(誤言語による全滅の解消) |
| **P1-β** | + B/C/D(n-best + LLM 訂正 + 語彙バイアス) | R-WER が低下。全体 WER は非悪化(訂正の副作用で悪化させない) |
| **P1-γ** | + E(NLLB へ接続)+ フォールバック総点検 | 翻訳段の入力品質(人手評価 or BLEU 代理)が向上。回帰ゼロ |
