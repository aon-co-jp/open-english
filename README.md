...
## Android版(単体動作、2026-08-11)

`android/`は、PC/LinuxのWEBサーバー無しで単体動作するAndroidアプリ。
`open-english-server`(静的フロントエンド配信)をクロスコンパイルして
APKへ同梱し、端末上の`127.0.0.1`で自己完結する。**正直な開示**: AI応答
エンジン(`aruaru-llm`)はまだ同梱していないため、チャット機能自体は
別途`aruaru-llm`を起動する必要がある(このAndroid版単体では静的UIの
表示までが実証範囲)。詳細・実機検証結果は`CLAUDE.md`のHANDOFF参照。
