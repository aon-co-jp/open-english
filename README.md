...
> 📌 **最新の更新(2026-09-07): 「PC版を起動してご利用下さい」バナーを
> 共有デモ環境限定で追加**。ブラウザ版(VPS等の共有環境で
> `localhost`/`127.0.0.1`以外のホスト名からアクセスした場合)にのみ、
> 「🖥️ PC版を起動してご利用下さい。」という固定メッセージを表示する。
> ローカルPC版(既にインストール済みのサーバーへ`localhost`/
> `127.0.0.1`で接続している場合)では表示しない——PC版利用者自身に
> 「PC版を起動してください」と案内するのは無意味なため。実機ブラウザで
> `127.0.0.1`アクセス時にバナーが非表示になることを確認済み。
>
...
> (起動時に加え、起動直後の「メンテナンス表示中」フェーズは**30秒ごと**に
> 本体＋関連リポジトリ全て〈aruaru-llm・aruaru-db〉をチェックして自動
> バージョンアップ〈最大約3分/6回、環境変数
> `OPEN_ENGLISH_MAINTENANCE_FAST_TICKS`で調整可〉。以降は**30分ごと**。
> 2026-09-09に定常間隔を6時間→30分へ短縮、2026-09-10にメンテナンス中
> 30秒フェーズを追加)+手動
> ダウングレード機能を追加しました。
...
|---|---|---|---|---|
| 1 | **open-english本体**(このリポジトリ、静的フロントエンド+配信サーバー) | [open-english-installer.exe](https://github.com/aon-co-jp/open-english/releases/latest/download/open-english-installer.exe) | [tar.gz](https://github.com/aon-co-jp/open-english/releases/latest)(`open-english-installer-<os>.tar.gz`) | [APK](https://github.com/aon-co-jp/open-english/releases/latest)(スマホ版`open-english-android-phone.apk`・タブレット版`open-english-android-tablet.apk`) |
| 2 | **aruaru-llm**(AI応答エンジン、必須——無いとチャット機能が動きません) | [aruaru-llm-installer.exe](https://github.com/aon-co-jp/aruaru-llm/releases/latest/download/aruaru-llm-windows-x86_64-installer.exe) | [tar.gz](https://github.com/aon-co-jp/aruaru-llm/releases/latest) | Android版は同梱済み(open-englishのAPK内に含まれる、別途インストール不要) |
