...
- **2026-08-25(続き4) リミット接近のためチェックポイント記録
  ...
  2. **正直な開示・未実施の検証(次回最優先で実施すること)**:
     (a) フォルダブラウザモーダルの実ブラウザ操作(`GET /v1/fs/list-dir`
     とのフロントエンド接続)は、このセッションのブラウザツールが
     ポップアップに繰り返しブロックされたため**未検証**——次回、
     実サーバーを起動しGUIで実際にドライブ→フォルダを辿って選択できる
     ことを確認する必要がある。(b)「既にインストール済み」バナーの
     表示切り替え(`checkHealth()`成功/失敗との連動)も同じ理由で
     **未検証**。(c) 一行コピペスクリプトの中身自体(各行のコマンドが
     実際に通しで実行可能か)は、このセッションでは新規に書き下ろした
     テキストの構成のみを確認しており、**実端末での実行検証はしていない**。
     (d)「Updates & Rollback」パネルが読み込み中のまま止まって見える、
  ...
  3. **今回未着手のまま持ち越した新規要望(コード変更は一切していない)**:
     ...
     パターンをそのまま踏襲できる)。
  4. **Android側ミラー(`android/app/src/main/assets/webroot/`)への
     同期コピーも今回は未実施**(次回、上記1番の検証完了後にまとめて
     反映すること)。
  - 次にすべきこと: (1) 上記2番(a)〜(d)の実機ブラウザ検証、
    (2) 検証後、Androidミラーへの同期コピー、(3) 上記3番(a)(b)の
    新規講座追加、(4)`cargo test --release`での既存テストの非回帰確認
    (今回は`main.rs`変更後の`cargo build --release`成功のみ確認済みで、
    テストスイートの再実行はしていない)。
