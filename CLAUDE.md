...
> **📌 2026-09-02追記: Model Foldingの「fold-layersを実際に呼び出す
> ボタン」をUIへ追加(ユーザー指示「open-english側UI実装」への対応)**
>
> 従来`index.html`の「⚙ Setup aruaru-llm」パネル内には
> `POST /v1/models/fold-layers`のAPI仕様の開示文のみがあり、実際に
> ボタンを押して呼び出す導線が無かった(`aruaru-llm/CLAUDE.md`
> 2026-09-01エントリ参照)。今回、既存の`apiBaseEl`(aruaru-llmサーバー
> URL欄)をそのまま再利用する形で、除去層数・線形アダプタ使用有無・
> `ridge_lambda`(2026-09-01付で既にAPI側は外部指定対応済み、詳細は
> `aruaru-llm/CLAUDE.md`参照)を入力できるフォーム+実行ボタンを追加した
> (`index.html`の新設`#fold-layers-*`要素、`app.js`の`foldLayersBtn`
> クリックハンドラ)。押下後、折りたたみ前後の層数・除去した層の
> インデックス・折りたたみ前後の生成サンプル・品質見込み
> (`quality_hint`)をステータス欄に表示する。`node --check app.js`で
> 構文確認済み。**実機検証は未実施**(この開発機に稼働中の
> `aruaru-llm`インスタンスが無いため、実際にボタンを押してHTTP応答が
> 返ることの確認は次回持ち越し)。
>
> **今回のタスクで指示された残り3項目についての正直な状況**:
> - **GPU実測(open-cuda側)**: このセッションの実行環境にGPU・
>   GPUドライバが無く、実際にVulkan/DirectX経由のベンチマークを
>   ビルド・実行することはできなかった(試すこと自体を試みたが、
>   `nvidia-smi`相当のGPU検出手段がこの環境に存在しない)。
>   `open-cuda/CLAUDE.md`には既に開発機(NVIDIA GT 730)での実測結果が
>   複数記録されており、そちらが正本。
> - **多言語較正**: 調査の結果、線形アダプタ方式のridge_lambda外部化
>   (今回のタスク項目4)は`open-cuda-llm`側で**既に実装・テスト済み**
>   (2026-09-01付、`fold_block_with_linear_adapter`の
>   `ridge_lambda: Option<f32>`引数、`fold-layers`HTTPリクエストの
>   `ridge_lambda`フィールド、`ridge_lambda_used`をレスポンスへ反映、
>   単体テスト4件——既定値へのフォールバック・明示指定の反映・
>   非正数値の拒否・実際にフィット結果が変わることの検証、いずれも
>   `open-cuda/crates/open-cuda-llm/src/lib.rs`に実装済み)。多言語
>   (日本語等)較正プロンプトの追加・実行による品質検証は、この
>   セッションでは着手しなかった(GPU無し環境のため実GPT-2重みでの
>   検証コマンドを実行できず、机上の変更のみになることを避けた)。
> - **ridge_lambda外部化**: 上記の通り既に完了済み(今回このセッション
>   での新規実装ではない)。
>
> 次にすべきこと: (1) 稼働中の`aruaru-llm`インスタンスがある環境で
> 今回追加したUIボタンの実機HTTP検証、(2) GPU搭載機での
> Vulkan/DirectXベンチマーク実測、(3) 日本語・他言語較正プロンプトの
> 追加と実GPT-2重みでの折りたたみ後品質検証。

> **📌 2026-09-01追記: フリーランス開発コーナーに「産業・カテゴリ」選択
> (誤字脱字補正+カスタム追加)を実装(ユーザー指示、段階1)、および今回
> 提示された大規模構想の棚卸し**
>
> **今回実装した部分(実機検証済み)**: `index.html`の
> `#freelance-corner-modal`に「産業・カテゴリ」の`<select>`+自由入力欄
> (`datalist`付き)+「＋ このカテゴリを追加」ボタンを新設。`app.js`に
> `FREELANCE_INDUSTRIES_DEFAULT`(EC/SaaS/フィンテック等30種)、
> 自前実装のLevenshtein距離による`freelanceSuggestIndustry`(入力の
> 誤字脱字を「もしかして」候補として提案、外部ライブラリ非依存)、
> `freelanceLoadCustomIndustries`/`freelanceSaveCustomIndustries`
> (localStorageで端末ごとに追加カテゴリを永続化)を追加し、
> `freelanceBuildJobSearchUrl`(求人検索)と「AI先生に相談」の質問文
> 組み立てへ選択した産業・カテゴリを組み込んだ。**実ブラウザ
> (python -m http.serverでのローカル配信+Claude Browser)で確認**:
> セレクトへ31件(既定30+未選択)が描画されること、「フィンテク」
> という誤字入力に対し「フィンテック / 金融」が正しく提案される
> こと、新規カテゴリ追加がlocalStorageへ永続化されセレクトへ反映
> されること、コンソールにこの新機能起因のエラーが無いことを確認済み
> (`node --check app.js`でも構文確認済み)。
>
> **今回のユーザー指示で追加提示された、まだ未着手の大規模構想
> (正直な棚卸し、実装はしていない)**: (1) カテゴリ一覧をメンテナンス
> 時にGoogle検索で自動的に最新化する仕組み、(2) 開発案件内容を
> 様々なフォーマットでアップロード、(3) VPSへの自動読み書き、
> (4) GitHubへの自動読み書き(操作)、(5) ローカルドライブへの自動
> 読み書き、(6) タイピング練習モード(ひらがな・カタカナ・漢字・
> アルファベット・数字テンキー・記号)、(7) マウス操作練習モード、
> (8) (6)(7)を含む初心者向けパソコン教室一式、(9)「プログラム
> レッスンを受けながら一緒に開発する」チェックボックスと、それに
> 連動したAI先生との共同開発フロー、(10) Visual Studio Code拡張機能の
> 新規開発、(11) 上記(3)〜(5)や外部AI(Google検索/GitHub調査/
> ChatGPT/Gemini/DeepSeek/Claude)によるフォローサポートを、世界中の
> 言語(ユーザーが選択した言語)+日英で説明する多言語対応。
>
> **正直な技術的所見(実装前に必ず踏まえること)**:
> - (3)(5)「VPS/ローカルドライブへの自動読み書き」は、ブラウザが
>   生のSSHソケット・任意のローカルファイルシステムパスを直接開けない
>   という構造的制約により、既存のGitHub連携(`freelanceGithubCreateRepoAndPush`、
>   ブラウザから直接REST APIを呼ぶ方式)と同じ設計は使えない。実現するには
>   サーバー側(`open-english-server`)で認証情報(SSH鍵・VPS接続情報等)を
>   安全に扱う新しい仕組みが必要——これは前回・前々回のHANDOFF
>   (2026-08-26)でも既に「未着手・セキュリティ設計が先」と明記済みの
>   ギャップであり、今回のユーザー指示で解消されたわけではない。
>   ローカルドライブについては、File System Access API(Chromeベース
>   ブラウザで利用可、ユーザーが明示的にディレクトリを選択する必要が
>   あり、任意の隠しパスへの無断アクセスはブラウザの権限モデル上
>   不可能)が現実的な選択肢だが、これも未着手。
> - (10) VS Code拡張機能は、open-englishのWebアプリとは別のnpm/TypeScript
>   ベースの新規開発物(VS Code Extension API)であり、規模として独立した
>   新規リポジトリ相当。既存のブラウザ内AI先生機能とどう連携させるか
>   (拡張機能からローカルのopen-english-serverへHTTPで問い合わせる、等)
>   の設計自体が未着手。
> - (6)(7)(8)「タイピング練習・マウス練習・パソコン教室」は英会話学習と
>   いうこのリポジトリの主目的から離れた別分野の機能であり、
>   前回セッションの棚卸し調査でも「独立した新規リポジトリ、または
>   既存の教育系リポジトリでの新設を検討する方が筋が良い」という所見を
>   記録済み。ユーザーの意向を確認しないまま、この場で新規リポジトリを
>   勝手に作成することはしない(`open-raid-z/CLAUDE.md`の「フォルダ
>   作成前に相談」方針に基づく)。
> - これら(3)(5)(6)(7)(8)(10)は、いずれも認証情報の安全設計・新規
>   リポジトリの要否判断など、ユーザーとのすり合わせ、または複数
>   セッションにまたがる規模の実装が必要——今回はそのうち安全に1パスで
>   実装・検証できる範囲(産業・カテゴリ選択)のみに絞って実装した。
>
> **次にすべきこと(優先順)**: (a) タイピング練習・マウス練習・
> パソコン教室機能を`open-english`本体に含めるか、独立リポジトリに
> するかをユーザーに確認する、(b) VPS/GitHub/ローカルドライブへの
> 自動読み書きの認証情報の安全設計(保存場所・暗号化・スコープ)を
> ユーザーとすり合わせる、(c) カテゴリ一覧のメンテナンス時Google検索
> 自動更新の実現方式(ビルド時スクリプトかランタイムか)を決める、
> (d) VS Code拡張機能の要件定義(何をどう連携させるか)を固める。
>
> ---

> **📌 2026-08-27追記(続き16・BUG修正): インストーラーのショートカット
> 作成が`IPersistFile::Save`エラー(0x80070003)で失敗する重大バグを
> ユーザー報告(スクリーンショット)で発見・修正**
>
> ユーザーから「open-english バージョン0.1.0 セットアップ」の実際の
> インストール画面のスクリーンショットが送られ、「ショートカットを
> 作成しています…Manage related tools \ 関連ツールの管理.lnk」の
> 表示中に「セットアップ: IPersistFile::Save エラー: コード
> 0x80070003. 指定されたパスが見つかりません。」というエラーダイアログ
> が出ている実際の不具合を報告いただいた。
>
> **原因調査**: `installer/windows/open-english.iss`の`[Icons]`
> セクション、`Name: "{group}\Manage related tools \ 関連ツールの
> 管理"`という行を発見。**Inno SetupのNameパラメータ内の`\`は、単なる
> 日英併記の視覚的な区切り文字ではなく、スタートメニューの
> 「サブフォルダ区切り」として解釈される仕様**であり、この行は
> 「`Manage related tools `(末尾に半角スペース付き)というサブ
> フォルダの中に`関連ツールの管理.lnk`を作る」という意図しない構造に
> なっていた。このサブフォルダ作成(または末尾スペースを含むフォルダ名)
> がWindows側のパスAPIで失敗し、`IPersistFile::Save`エラー
> (`ERROR_PATH_NOT_FOUND`)を引き起こしていたと判明。
>
> **修正の経緯(1回目は不十分だった、正直な記録)**: 最初`\`を`/`
> (スラッシュ)へ変更して`ISCC.exe`で再ビルドし、実機でサイレント
> インストール(`/VERYSILENT /LOG=...`)を行って検証したところ、
> **`/`もWindowsのパス正規化により`\`と同様に扱われ、依然として空の
> サブフォルダ`Manage related tools`が作成される**ことが実機ログで
> 判明した(1回目の修正は不十分だった)。パス区切りとして一切解釈
> されない表記が必要と判断し、最終的に`"Manage related tools
> (関連ツールの管理)"`という、丸括弧で日本語訳を併記する形へ変更した。
>
> **実機検証(2026-08-27、`ISCC.exe`でのビルド+`/VERYSILENT`での
> サイレントインストールを複数回実施)**: 修正版をビルドし、実際に
> ユーザープロファイル配下へサイレントインストールを実行、インストール
> ログに`Successfully created the icon.`(成功)が記録されること、
> スタートメニューフォルダ内に`Manage related tools (関連ツールの
> 管理).lnk`という**フラットな(サブフォルダの無い)**ショートカット
> ファイルが作成されることを確認した。テスト用インストール(レジストリ
> キー・フォルダ・実行中プロセス含む)はすべてクリーンアップ済み。
> リポジトリ直下の`installer/windows/open-english-installer.exe`
> (2026-08-26にコミットされていた旧バージョン)も、修正版でビルド
> し直したバイナリへ差し替えた。
>
> ---
>
...
>
> ユーザーからの質問をきっかけに、`create-vault-incognito-shortcut.
> ps1`(続き10で新設)に設計上の欠陥があることに気付いた。
>
> **欠陥の内容**: 従来のスクリプトはショートカットの起動先を
> `vault.html`単体のURL(`http://127.0.0.1:4601/vault.html?
> parentOrigin=...`)にしていた。しかし`vault.html`は**本体ページ
> (index.html)に埋め込まれたiframeとしてのみ**正しく機能する設計
> (GitHub push・Google検索の代行実行はpostMessageで親ページと
> やり取りする)——`vault.html`を単体の(iframeではない)トップレベル
> ページとして直接開くと、**そもそも親ページが存在しないため
> `event.source`が無く、GitHub push・Google検索の代行実行機能が
> 動作しない**(トークンの暗号化・保存・復号自体は単体でも動作する)。
>
> **修正内容**: ショートカットの起動先を`vault.html`単体から**本体
> アプリ(index.html)のトップページ**(`http://127.0.0.1:4601/`)へ
> 変更。ユーザーはシークレットウィンドウで開いた本体アプリの中から、
> フリーランス開発コーナーやGoogle検索設定を開いて`vault.html`のURLを
> 指定し、正しくiframeとして埋め込む流れになる。また、パラメータ名を
> `-VaultUrl`から`-TargetUrl`へ変更し、`-ServerHost`/`-ServerPort`
> という新しい引数でローカルのアドレス(既定は`127.0.0.1:4601`)を
> 上書きできるようにした(同一LAN内の別端末からアクセスする場合や、
> 既定と異なるポートで動かしている場合に対応)。`vault.html`の案内文
> にもこの制約・使い方を追記。
>
> **実機検証(2026-08-27)**: PowerShellパーサーで構文確認、既定の
> ローカルアドレスでスクリプトを実行し`http://127.0.0.1:4601/`を
> 起動先とするショートカットが生成されること、`-ServerHost`/
> `-ServerPort`で任意のアドレス(例: `192.168.1.20:4601`)を指定した
> 場合にそのアドレスが正しく反映されることを、生成された.lnkファイルの
> `Arguments`を実際に読み出して確認済み(テスト用ショートカットは削除)。
>
> ---
>
> **📌 2026-08-27追記(続き13・BUG修正): 検索付きプロンプトに「質問の中に
> 別の生成キューが入れ子になる」品質バグを発見・修正(実機E2Eテストで
> 発見)**
>
> ユーザー指示「実用的になるまで数回繰り返して」に沿った追加の実機
> テストで、モックした検索成功レスポンスを使って実際に`/v1/generate`
> へ送られるプロンプトの中身を検証したところ、品質上の不具合を発見した。
>
> **不具合の内容**: `buildSearchAugmentedPromptClient(context, prompt)`
> の第2引数に、`prompt`(メイドカフェ講師ペルソナ・レベル指示・
> 「Student: {発話}\nTrainer:」まで組み込んだ、既にラップ済みの
> テンプレート全文)をそのまま渡していたため、実際に送られていた
> プロンプトは:
> ```
> Question: You are a friendly English conversation trainer at a maid
> cafe. ... Student: what is rust programming\nTrainer:
> Answer:
> ```
> という、**「質問」の中にペルソナ全文と別の生成キュー(`Trainer:`)が
> 入れ子になり、最後に`Answer:`が続くという分かりにくい構造**になって
> いた(`Trainer:`と`Answer:`という2つの生成キューが連続する)。
>
> **正直な補足**: この「ラップ済みプロンプトをそのまま『質問』として
> 渡す」という構造自体は、実は今回新規に作った`vault`/own-key経路
> だけの問題ではなく、既存の(共有サーバー設定を使う)
> `/v1/generate-with-search`経路(`aruaru-llm`側の
> `build_search_augmented_prompt(context, req.prompt)`)にも**元から
> 同じ構造で存在する**——`req.prompt`として送られる値自体が既にラップ
> 済みのため。今回は自分が完全にコントロールできるクライアント側の
> 新経路(own-key/vault)のみを修正した。**サーバー側の既存経路も同様の
> 修正が必要かは次回検討**(サーバー側は`req.prompt`しか知らず、
> ラップ前の生発話とラップ後のテンプレートを区別できないため、
> クライアント側で生発話を別フィールドとして送るAPI変更が必要になる
> 可能性がある)。
>
> **修正内容**: `buildSearchAugmentedPromptClient`の第2引数を`prompt`
> から`userText`(利用者が実際に入力した生の発話のみ)へ変更。
> 修正後は`Question: what is rust programming\nAnswer:`という、
> 意図通りのシンプルな構造になる。
>
> **実機検証(2026-08-27)**: `googleSearchRequestVault`を成功レスポンス
> を返すようモックした上で実際にメッセージ送信を実行し、`/v1/generate`
> へ送られたリクエストボディを`fetch`インターセプトで直接確認。修正前は
> ペルソナ全文が「質問」に入れ子になっていること、修正後は
> `Question: what is rust programming\nAnswer:`という正しい構造に
> なっていることの両方を確認した。
>
> **次にすべきこと**: `aruaru-llm`側の`/v1/generate-with-search`
> (共有サーバー設定を使う経路)にも同様の「質問の入れ子」問題がある点を、
> クライアント側で生発話を分離して送る設計変更を含めて検討する。
>
> ---
>
> **📌 2026-08-27追記(続き12・防御的修正): GitHubトークン側にも同種の
> フォールスルーを防御的に修正(横展開)**
>
> 直前のエントリ(続き11)でGoogle検索④vaultモードに見つけたのと同じ
> パターン(`loadOwnGoogleSearchCredentials`が④vaultモード専用の分岐を
> 持たず、③〈plain〉と同じlocalStorageチェックへフォールスルーする
> バグ)が、GitHubトークン側の`freelanceLoadGithubToken`にも存在する
> ことを横展開で確認した。**現状の唯一の呼び出し元
> (`freelanceGithubCreateRepoAndPush`)はvaultモード時には呼ばれない
> 設計のため実害は無い(デッドコード)**だが、将来の呼び出し追加に
> 備えた防御的修正として、`mode === "vault"`の場合は無条件に空文字列を
> 返すよう修正した。
>
> **実機検証(2026-08-27)**: ブラウザで③平文モードでトークン保存→
> ④vaultモードへ切替→`freelanceLoadGithubToken()`を直接呼び出して
> 空文字列が返ること(修正前は平文トークンが返っていたはず)を確認。
>
> ---
>
> **📌 2026-08-27追記(続き11・BUG修正): Google検索④vaultモードの2件の
> バグを実機テストで発見・修正(ユーザー指示「実用的になるまで数回
> 繰り返して」への対応)**
>
> vault機能一式を実装した後の品質確認パスで、実際にブラウザ操作を
> 再現して2件のバグを発見・修正した(いずれも実害は限定的だが、
> 「vaultモードでは本体ページに一切キーを渡さない」という設計原則に
> 反する不整合だったため修正)。
>
> 1. **バグ: `loadOwnGoogleSearchCredentials()`がvaultモードで
>    フォールスルーしていた**。①②(file/encrypted)は専用分岐が
>    あったが、④(vault)には専用分岐が無く、③(plain)と同じ
>    `localStorage`チェックへ落ちていた。このため、**以前③モードで
>    平文保存したキーが残っていると、④vaultモードを選んでいても
>    その平文キーが本体ページのJSメモリへ読み込まれてしまう**(実際の
>    検索処理はvault経由へ切り替わるため、そのキーが外部送信される
>    実害までは無かったが、「vaultモードでは本体ページに一切キーを
>    渡さない」という設計原則に反する不要な露出だった)。`mode ===
>    "vault"`の場合は無条件に`null`を返すよう修正。
> 2. **バグ: `refreshGoogleSearchStatus()`がvaultモードを考慮していな
>    かった**。vaultモードで実際にキーを復号済みでも、本体モーダルの
>    ステータス文言が「⚪ Not set yet — search will not run for you」
>    (未設定)のままになる誤解を招く表示だった。vaultモード用の分岐を
>    追加し、「vault欄をご確認ください」という案内へ変更。
>
> **実機検証(2026-08-27)**: ブラウザで(a)③モードでキーを保存→
> (b)④vaultモードへ切り替え→(c)`loadOwnGoogleSearchCredentials()`を
> 直接呼び出して`null`が返ることを確認(修正前は平文キーが返っていた
> はず)、(d)モーダルのステータス文言が正しく「vaultモード使用中」へ
> 変わることを確認。また、vault未読み込み状態で検索付き送信を行った
> 場合に例外を投げず正しいエラーメッセージ(「Vaultが読み込まれて
> いません」)で失敗することも確認。
>
> ---
>
> **📌 2026-08-27追記(続き10・実装): Chromeシークレットモード強制起動
> ショートカット+ダウンロードリンクを追加(ユーザー指摘を受け訂正)**
>
> ユーザーから「あらかじめChromeをインストールしておけば、アイコンを
> ダブルクリックで起動時もChromeのシークレットモードで強制的に開くよう
> にできる」との指摘があり、前回エントリの「ブラウザ・モードの強制
> 切り替えは不可能」という説明が**不正確だった**ことを認め、訂正した。
> **正確には**: 「Webページ自身がJavaScriptから強制することは不可能」
> であって、「OS側のショートカット(アイコン)がブラウザの実行ファイルを
> コマンドライン引数付きで起動する」のは別の話であり、これは**可能**
> (Chromeの`--incognito`起動オプションは実在する標準機能、裏技ではない)。
> Web調査で構文(`"chrome.exe" --incognito <URL>`、`--incognito`は
> クォート外に置く必要がある)を確認した上で実装した。
>
> **実装**:
> 1. `launchers/windows/create-vault-incognito-shortcut.ps1`新設:
>    Chromeの実インストール先(Program Files/Program Files (x86)/
>    LocalAppData配下を順に確認)を検出し、見つかれば`vault.html`を
>    `--incognito`付きで開くデスクトップショートカット(.lnk)を作成
>    する。**正直な設計判断**: Chromeが見つからない場合、他ブラウザの
>    プライベートモードへ黙って代替しない(ブラウザ間でシークレット
>    モードの挙動が異なりすぎるため、黙った代替は誤解を招く)——
>    Chromeのダウンロードを促すメッセージを表示するのみ。
> 2. `vault.html`にGoogle Chromeの公式ダウンロードリンク(日英併記)と、
>    上記スクリプトの案内を追加。
>
> **実機検証(2026-08-27)**: PowerShellのパーサー(`[System.Management.
> Automation.Language.Parser]::ParseFile`)で構文エラー無しを確認。
> **実際にこの開発機でスクリプトを実行し、本物のデスクトップショート
> カット(.lnk)が生成されることを確認**——`WScript.Shell`で生成された
> .lnkファイルの`TargetPath`/`Arguments`を読み出し、
> `TargetPath: ...\chrome.exe`・`Arguments: --incognito "http://127.0.0.1
> :4601/vault.html?..."`と、意図通りの内容になっていることを確認した上で
> テスト用ショートカットは削除した。ブラウザ側(vault.html)のダウンロード
> リンク・スクリプト案内文の表示も確認済み。
>
> ---
>
> **📌 2026-08-27追記(続き9・実装): シークレットモード案内+「シークレット
> モード+①ファイル方式」を推奨組み合わせとして`vault.html`に追加**
>
> ユーザー提案「ブラウザをChromeに強制し、強制的にシークレットモードへ
> 変更して利用する」への対応。**正直な技術調査結果**: Webページから
> (1)利用ブラウザをChromeへ強制切り替え、(2)シークレット/プライベート
> モードへ強制切り替え、のいずれも**技術的に不可能**(ブラウザベンダー
> がプライバシー保護のため意図的にブロックしている仕様、回避策は無い)。
> 実現できるのは推測ヒント表示と手動切り替えの案内のみと判断し、
> その範囲で実装した。
>
> **実装(`vault.html`)**:
> 1. `navigator.storage.estimate()`のクォータ(シークレットモードは
>    通常モードよりクォータが大幅に小さいことが多い、Chrome既知の挙動)
>    を使ったシークレットモード推測ヒューリスティックを追加(100%正確
>    ではない旨を明記、「断定」ではなく「推測」として表示)。
> 2. Chrome以外のブラウザでは「シークレット判定の仕組みが最も広く
>    検証されているのはChrome」という推奨メッセージを表示(ブラウザの
>    強制切り替え自体は不可能なため、推奨に留める)。
> 3. **矛盾の指摘と解決**: ユーザーとの議論で「②パスフレーズ暗号化
>    (localStorageへ永続保存)は、シークレットモード(ウィンドウを
>    閉じるとlocalStorageごと消える)とは目的が根本的に矛盾する」
>    ことが判明した——②の価値は「次回も残っている」利便性、シークレット
>    モードの価値は「痕跡を残さない」ことで、正反対の設計思想。一方
>    ①ローカルファイル方式はそもそも何も保存しない設計のため、
>    シークレットモードと矛盾しない。この分析に基づき、**「シークレット
>    モード+①ファイル方式」を推奨組み合わせとして明記**し、`vault.html`
>    の`<select id="mode">`の既定値も②から①へ変更した(従来は②が
>    最初のoption=既定だったが、①を最初のoption+`selected`属性へ変更)。
>
> **実機検証(2026-08-27)**: `node --check`でスクリプト部分の構文確認、
> ブラウザで既定選択が①になっていること・推奨組み合わせの案内文が
> 正しく表示されること・GitHubトークンの暗号化保存(既存機能)に
> 回帰が無いことを確認済み。
>
> ---
>
...
>
> ユーザーから「aruaru-llmなどをキー受け渡し時だけOFFにするアイデアは
> どうか」との相談があり、正直に回答した(GitHubトークンはvault.html内
> で完結し`aruaru-llm`は最初から無関係、Google検索キーも暗号化・復号は
> ブラウザ内で完結し`aruaru-llm`が関与するのは実際に検索付き生成を使う
> 瞬間のみ、という現状の設計を説明)。その上で「ブラウザ側で必要な所
> だけON/OFFできないか」との提案を受け、既存の`#web-search-toggle`
> (Google検索補強チェックボックス、既定OFF)が既にこの目的に近いことを
> 確認・説明し、**「ONにしても次のメッセージには引き継がれず、送信の
> たびに自動でOFFへ戻る」**という強化を追加実装した(合意の上)。
>
> **実装(`app.js`)**: `useWebSearch`を読み取った直後、fetch開始前に
> `webSearchToggleEl.checked = false`でチェックボックスをリセットする
> (ネットワーク失敗時でもON状態が残らないよう、レスポンス待ちより
> 先にリセットする設計)。`index.html`のラベルにも
> 「(auto-off after each send) / (送信ごとに自動OFF)」を明記し、
> `title`属性でホバー時の説明も追加。
>
> **正直な効果の範囲**: これは`aruaru-llm`プロセス自体を止める仕組み
> ではなく、**ブラウザ側が「今回のリクエストにキーを含めるかどうか」を
> 判断する条件を、より頻繁に(メッセージ毎に)明示的な同意へ戻す**という
> 変更に過ぎない。ONの状態でメッセージを送った、その1回のリクエストで
> `aruaru-llm`へキーが渡ること自体は変わらない(この設計変更は「渡す
> 頻度・意図の明確さ」を上げるものであり、「渡らなくする」ものではない
> ——渡らなくするには前回エントリで議論した「Google検索もブラウザから
> 直接呼ぶ」設計が必要、こちらは引き続き次回タスク)。
>
> **実機検証(2026-08-27)**: `open-english-server`をローカル起動し、
> チェックボックスをON→メッセージ送信→(非同期処理の完了を待って)
> チェックボックスが自動でOFFに戻ることをJavaScript実行で確認済み。
>
> ---
>
> **📌 2026-08-27追記(続き5・実装): 2つのメールアドレスによるOTPログイン
> (バックアップ用email2)を実装**
>
> ユーザー指示「email1とemail2の入力フォームを作って、この二つの
> メールアドレスとワンタイムパスワードを使ってログインする新しい
> セキュアなシステムにお乗り換え下さい」への対応。事前の
> `AskUserQuestion`で3方式(バックアップ/二段階認証/二人承認)を提示し、
> **「両方に同じOTPコードを送り、どちらか一方で入力できればログイン
> 成功(バックアップ用)」**を選択いただいた。
>
> **実装**:
> 1. `server/src/auth.rs`の`request_otp`を`request_otp(email1, email2:
>    Option<&str>)`へ変更。同一の6桁コードを`email1`(必須)・`email2`
>    (任意、指定時のみ)の両方へ送信し、`OtpEntry`を両方のメール
>    アドレスキーで保存する(`verify_otp`はどちらのメールアドレスで
>    呼んでも成功する——コード自体は共通のため)。`email2`が空文字列・
>    `email1`と同一(大文字小文字無視)の場合は送信をスキップする
>    重複排除ロジックを追加。
> 2. `server/src/main.rs`の`RequestOtpRequest`に`email2: Option<String>`
>    (`#[serde(default)]`、後方互換——`email2`を送らない既存クライアント
>    もそのまま動く)を追加。
> 3. `index.html`の`#login-gate`・初回案内(`#login-setup-prompt`)双方に
>    「新しいセキュアなシステムへお乗り換えください」という日英併記の
>    案内文を追加し、Email 2(任意)の入力欄を新設。コード確認画面には
>    「どちらのメールアドレスで受け取ったか」を選べる欄
>    (`#login-verify-email`、既定値はEmail 1)を追加。
> 4. `app.js`側で`email2`を`request-otp`へ同梱、検証時は
>    `#login-verify-email`の値(利用者が実際にコードを受け取った方の
>    メールアドレス)を送信するよう変更。
>
> **正直な開示(誤解防止のため必須)**: これは**二段階認証ではない**
> ——email2は「本人が受け取れる経路を増やすための予備」であり、
> 片方だけ届けば認証を突破できる。単一メールのみの場合と比べて
> セキュリティが「強化」されるわけではなく、あくまで**可用性**
> (メインのメールが使えない時の代替経路)の改善である。この点を
> UI・コードコメント双方に明記した。
>
> **実機検証**: `cargo build --release`成功後、ローカルサーバーで
> `POST /v1/auth/request-otp`に`email`+`email2`を渡すリクエストが
> 正しく受理され(SMTP未設定のためのHTTP 503は想定通り、
> リクエストボディのデシリアライズ自体が失敗していないことを確認)、
> `#login-email2`・`#login-verify-email`のDOM要素が実際に存在する
> ことをブラウザで確認済み。`auth.rs`に単体テスト2件追加
> (email2の不正フォーマットが早期に拒否されること、email1と同一の
> email2が重複排除されSMTP設定エラーまで正常に到達すること)——
> **実際にSMTPサーバーを使った実メール送信のE2E検証は、この開発機に
> SMTPサーバーが無いため未実施**(正直な開示)。
>
> ---
>
> **📌 2026-08-27追記(続き4・次回タスク記録、未着手): ログイン必須化は
> VPS/共有サーバー環境限定で導入する方針で合意、多言語調査の追加結果**
>
> ユーザーから「セキュアにログインを絶対に行なってから開始した方が
> 良さそうならして」との相談があり、以下の方針を提案し合意を得た
> (実装は次回、今回はセッション終了のため記録のみ):
>
> **ログイン必須化の方針**: 全ユーザーに一律強制するのではなく、
> **VPS/共有サーバーデプロイ環境でのみ必須化する**。個人PCでの単独
> 利用では他人がアクセスしない前提のため、ログイン必須化は恩恵の無い
> 手間の増加(かつメール送信環境の用意が動作要件に加わる)。一方
> easy-web.tokyo等の共有デプロイでは意味がある。**次にすべきこと**:
> `OPEN_ENGLISH_REQUIRE_LOGIN`のような環境変数(VPSデプロイ時のみ
> 設定)を新設し、設定時は`#login-gate`を常時表示(`login-setup-prompt`
> の「ログインせずに利用する」選択を無効化)する実装を行う
> (`server/src/main.rs`の設定読み込み+`app.js`側の分岐)。
>
> **多言語(日本語以外)でのGoogle/GitHub追加調査結果**(ユーザー指示
> 「世界中の言語でGoogle検索やGithub調査が必要ならして」への対応):
> 1. **DeepSeek APIのCORS対応(中国語で検索、確認完了)**: DeepSeek
>    公式APIもOpenAI/Gemini同様CORS未対応と確認(中国語の開発者
>    コミュニティ〈CSDN等〉でも「バックエンド経由が必須、フロント
>    エンドから直接呼べない」という報告が一致)。これで4大AIプロバイダ
>    全ての調査が完了——**Claude(Anthropic)のみブラウザ直接呼び出しに
>    対応、OpenAI/Gemini/DeepSeekはいずれも非対応**という結論が確定。
> 2. **Argon2id採用の検討(GitHubでOSS事例調査)**: 他のゼロ知識保管庫
>    OSS(`Vault-me`等)を調査した結果、多くが現行のPBKDF2ではなく
>    **Argon2id**(GPU/ASIC総当たり耐性が高いメモリハードな鍵導出関数)
>    を採用していると判明。ただしWeb Crypto APIはArgon2をネイティブ
>    対応しておらず、WASMライブラリ(`argon2-browser`等、ただし
>    最終更新が2021年頃で更新停滞気味——採用するなら`argon2ian`
>    〈8.5KB、より新しい〉等の代替も比較検討すべき)の追加が必要。
>    **これは「秘密を隠す」目的のWASM活用ではなく「ブラウザに無い
>    アルゴリズムを補う」目的であり、2026-08-27の別エントリで
>    「WASM化はセキュリティ改善にならない」と回答した趣旨とは異なる
>    正当な理由**(誤解防止のため明記)。外部WASM依存が1つ増える
>    (`vault.html`の「自己完結・外部依存無し」という設計方針との
>    トレードオフ、要検討)。
>
> **次にすべきこと**: (1) ログイン必須化の環境変数実装、(2) Argon2id
> 移行の要否をユーザーと相談(外部WASM依存を許容するかどうか)。
>
> ---
>
...
>
> ユーザーから2つの改善提案があり、いずれも「その場で安全に仕上げられる
> 小さな変更ではなく、設計をきちんと詰めるべき規模」と判断し、次回課題
> として記録する(今回は調査のみ、実装は無し)。
>
> **1. ログインセッションに紐づくサーバー側の暗号化保管庫**: 現行の
> パスフレーズ暗号化(2026-08-27導入、GitHubトークン・Google検索キー)
> はブラウザのlocalStorageのみで完結する設計(サーバーは一切関与しない)。
> ユーザー提案は、これを`#login-gate`(2026-08-26導入のメール+ワンタイム
> コードログイン)のセッションに紐づけたサーバー側の暗号化保管庫へ発展
> させるというもの。**メリット**: 複数端末間での同期、未ログインの
> 第三者からのアクセス制限。**必要な設計**: 新しいDBスキーマ(暗号化
> ペイロードの保管)・暗号化キー管理(サーバー側で復号する場合は
> サーバーがマスターキーを持つことになり、現行の「サーバーは秘密情報を
> 一切見ない」設計原則との整合を取る必要がある——例えばゼロ知識設計
>〈サーバーは暗号文しか見えず、復号はクライアント側のみで行う〉に
> するか、要検討)・新規APIエンドポイント。**次にすべきこと**: 保管庫の
> 「サーバーが復号鍵を持つか否か」という設計方針をユーザーと確認した
> 上で、スキーマ・エンドポイント設計から着手する。
>
> **1b. クロスオリジンiframeサンドボックス案(2026-08-27追記、上記1の
> 保管庫と組み合わせられる独立した改善案、ユーザー提案)**: 鍵の入力・
> 復号処理を、`open-english`本体とは**別オリジン**(例:
> `vault.open-english.example`のような別サブドメイン)に置いたiframe
> 内で行う設計。ブラウザの同一オリジンポリシーにより、本体側で万一
> XSSが起きても、攻撃者のJSはiframe内のDOM・localStorage・メモリへ
> 一切アクセスできない(Stripe Elements・Googleログインボタン等が
> 採用している実績のある手法)。本体とiframeは`postMessage`で最小限
> のやり取り(例:「復号済みトークンを1回だけ使わせて」)のみ行う。
> **検討したが効果が薄いと判断した代替案(誇張しないための記録)**:
> WASMモジュール化(同一オリジン内で動く限り、悪意あるJSがWASMの
> 公開関数を直接呼び出せるためXSS対策として弱い)、Web Worker化
> (別スレッドだが同一オリジンのため、乗っ取られたメインスレッドが
> `postMessage`で復号を依頼できてしまい同様に防げない)。
> **トレードオフ**: 別オリジンを1つ新たに用意・運用する必要がある
> (DNS・証明書・別サーバーまたは静的ホスティング)、iframe内UI/UXを
> 別途作る必要がある、iframe自体が乗っ取られた場合は防げない
> (ただし機能が絞られる分、攻撃対象範囲は本体より小さくなる)。
> **次にすべきこと**: 上記1のサーバー側保管庫の設計方針が決まった後、
> その保管庫UIをこのiframeサンドボックス内に置く形で統合設計する
> (両者は独立に着手可能だが、最終的には組み合わせるのが本命)。
>
> **2. Claude(Anthropic)キーのブラウザ直接呼び出し化**: 2026-08-27時点の
> Web調査(英語圏の一次情報)で、主要AIプロバイダのCORS対応状況を確認:
> - **Claude(Anthropic)**: `anthropic-dangerous-direct-browser-access:
>   true`ヘッダーでブラウザからの直接呼び出しに対応済み(Anthropic公式
>   が「dangerous」と命名——ブラウザに置いたキーが盗まれるリスクは
>   GitHub PATと同様に残る、誇張しない)。
> - **OpenAI(ChatGPT)**: CORS未対応、ブラウザから直接呼ぶとブロック
>   される(コミュニティで多数報告)。サーバー経由が必須。
> - **Gemini**: CORS未対応(`generativelanguage.googleapis.com`が
>   preflightで拒否)。サーバー経由が必須。
> - **DeepSeek**: 情報不足、実際にリクエストして検証する必要がある
>   (未確認のまま「対応/非対応」と決めつけない)。
>
> つまり4プロバイダのうちClaudeのみ、GitHub連携と同じ「ブラウザから
> 直接、サーバーを経由せず」という設計が技術的に可能。ただし現状は
> `chat_providers::complete_in_priority_order`(`aruaru-llm`側Rust、
> 複数プロバイダの優先順位・自動フォールバックを担う)がすべての
> プロバイダを一律にサーバー経由で扱っており、**Claudeだけを分離して
> ブラウザ直接呼び出しにする場合、この優先順位・フォールバックロジックの
> 一部をopen-english側(JS)へ移すか、Claude用に特別なパスを新設するかの
> アーキテクチャ設計が必要**。中途半端に手を入れると既存の複数プロバイダ
> フォールバックが壊れるリスクがあるため、今回は着手しなかった。
> **次にすべきこと**: (1) DeepSeekのCORS対応状況を実機で検証、
> (2) Claude分離のアーキテクチャ設計(`aruaru-llm`側のフォールバック
> ロジックとどう共存させるか)をユーザーと相談してから着手。
>
> ---
>
> **📌 2026-08-27追記(続き): Google検索APIキーへも同じ安全な受け渡し
> 方式を拡張、AIプロバイダキーは適用対象外と正直に判断、移行前の
> 秘密情報の破棄を案内(ユーザー指示、実機検証済み)**
>
> ユーザーから「Google検索などのAPI KeyやID、AIなどのKeyやIDも暗号化で
> 安全に受け渡し出来るように」との追加依頼、および「このセキュリティ
> システム導入前のVPS/レンタルサーバーの秘密鍵・公開鍵・GitHubトークン・
> API KeyやIDは破棄するように指導して」との依頼への対応。
>
> 1. **暗号化ヘルパーの汎用化**: 元はfreelance GitHubトークン専用だった
>    `freelanceEncryptGithubToken`/`freelanceDecryptGithubToken`を
>    `owEncryptSecret`/`owDecryptSecret`という汎用関数へ切り出し
>    (`app.js`)、他の秘密情報でも使い回せるようにした(ロジックの重複
>    を避けるためのリファクタリングのみ、暗号化方式自体は変更なし)。
> 2. **Google検索APIキー(`#google-search-modal`)**: GitHub連携と同じ
>    ①ローカルファイル読込(JSON形式`{"api_key":"...","cx":"..."}`)・
>    ②パスフレーズ暗号化・③平文保存、の3方式を追加。このキーは
>    ブラウザから直接使う(サーバーへは検索リクエストのたびにその
>    リクエスト限りで送るのみ、GitHub連携と同じ設計)ため、暗号化に
>    意味のある効果がある。
> 3. **AIプロバイダキー(ChatGPT/DeepSeek/Gemini/Claude、`#provider-
>    priority-modal`)は対象外と判断——正直な理由を明記**: これらの
>    キーは機能上**必ずaruaru-llmサーバーへ平文で送信される**設計
>    (サーバー側がそのキーで各AIプロバイダを呼び出すため)。ブラウザ
>    側のlocalStorageキャッシュを暗号化しても、送信先のサーバーで
>    平文として扱われる事実は変わらず、意味のある改善にならないと
>    判断し、**見せかけの安全対策を追加しないことを選んだ**(誠実さの
>    方針)。代わりに「信頼できるサーバーでのみ設定」「使用量上限つき
>    のキーを使う」「定期的にローテーション」を推奨する注記をUIへ追加。
> 4. **移行前の秘密情報の破棄案内**: 2026-08-26〜27の間、GitHub連携は
>    平文保存(③相当)しか選択肢が無かったため、その間に保存された
>    GitHubトークンは失効・再発行を推奨する注記を追加。あわせて、VPS/
>    レンタルサーバーのSSH秘密鍵・公開鍵についても(本アプリ自体は
>    SSH鍵入力機能を持たないが)、他の場面で平文保存していた場合の
>    一般的注意として、破棄・再発行を推奨する文言を追加。いずれも
>    日英併記。
>
> **実機検証(2026-08-27)**: Google検索キーの①②③モード切り替え・
> 暗号化保存→正しいパスフレーズでの復号一致・誤パスフレーズでの
> 失敗・ファイルモードでのトークン解決、および既存のfreelance GitHub
> トークン機能(共有関数へのリファクタリング後)が引き続き正しく動作
> することを、JavaScript実行で確認済み。
>
> ---
>
> **📌 2026-08-27追記: GitHubトークンの受け渡し方法を3種類に拡張
> (ユーザー指示「フォームより安全な受け渡し方法/暗号化を検討」への対応、
> 実機検証済み)**
>
> ユーザーから、フォームへの直接入力・保存(localStorage)より安全な
> 受け渡し方法(ローカルドライブ内ファイル経由)、または「ランダム要素の
> ある暗号化システム」の検討を求められた。**正直な技術判断として両方に
> 意味があると判断し、両方実装した**(誇張・安易な同意はしない方針):
>
> 1. **①ローカルファイルから読み込む(推奨・最も安全)**: トークンを
>    テキストファイルに保存しておき、使う都度`<input type="file">`で
>    選択。**一切保存しない**(メモリ上のみ、タブを閉じる/リロードで
>    消える)。localStorageより安全な理由: ブラウザを閉じた後にディスク
>    上へ平文が残らないため、後から漁られる保存領域が無い。
> 2. **②パスフレーズによるAES-GCM暗号化(利便性と安全性の折衷)**:
>    Web Crypto API(`crypto.subtle`)でPBKDF2(250,000回反復、SHA-256)
>    によりパスフレーズから鍵を導出し、**都度ランダムなsalt・iv**を
>    生成してAES-GCMで暗号化した上でlocalStorageへ保存。パスフレーズ
>    自体は保存しない。**正直な限界の開示(誇張しない)**: 「ランダム
>    要素のある暗号化」自体は本物の暗号化だが、**復号鍵(パスフレーズ)を
>    JS実行環境の外に安全に隔離する方法は無い**ため、ページがXSS等で
>    乗っ取られ、かつユーザーが既にパスフレーズを入力(復号)した後の
>    状態であれば、平文トークンを読まれるリスクは①と同程度に残る。
>    暗号化が守るのは「ブラウザを閉じた後・パスフレーズ入力前」の
>    保存領域のみ——**これを「暗号化したから安全」と過大に伝えることは
>    誠実さの方針に反するため、UIにも同じ限界を明記した**。
> 3. **③平文でlocalStorageに保存(非推奨、旧来の唯一の方式)**: 従来の
>    挙動を選択肢として残す(最も手軽、最も安全性が低い)。
>
> **実装箇所**: `index.html`の`#freelance-github-token-mode`セレクタ+
> 3つのセクション、`app.js`の`freelanceLoadGithubToken`(モード別に
> トークンを解決)・`freelanceDeriveAesKey`/`freelanceEncryptGithubToken`/
> `freelanceDecryptGithubToken`(Web Crypto API)・
> `freelanceGithubTokenFileBtn`のファイル読み込みハンドラ。
>
> **実機検証(2026-08-27、`open-english-server`起動+ブラウザJS実行)**:
> 暗号化→localStorageに保存された値が平文を含まないこと、正しい
> パスフレーズでの復号が元のトークンと完全一致すること、**誤った
> パスフレーズでは復号が失敗しエラー表示されること**、削除ボタンで
> 暗号化済みペイロードがlocalStorageから消えること、ファイルモードで
> 選択した内容がそのままトークンとして使われること、をすべて確認済み。
>
> ---
>
...
- **2026-09-08 VPS上の放置検証タイマーを削除(ユーザー指摘「WEB版の
  ドメイン/URLとWindows版のドメイン/URLは異なる様にしましょう」+
  「Windows版もWEB版もopen-easy-webを基本として、それぞれのドメイン/URLを
  有料ドメインでもDuckDNSの様な無料のドメインでも簡単登録機能を搭載」への
  対応・調査)**:
  1. **調査結果**: `easy-web.tokyo`(共有デモ、VPS自身のIP
     160.251.237.162)と`open-english.duckdns.org`(下記
     続き3で自己ホスト例として案内)は、DNS上は別IP(106.72.247.97)を
     指しており「ドメインが同一」という問題は無かった。しかし
     `open-english.duckdns.org`の実体は、続き3の実機検証時に**IPを
     ハードコードしたままのVPS上のsystemdタイマー
     (`duckdns-open-english.timer`/`.service`)**であり、2026-09-07の
     検証以降24時間以上、実際のPCの生存確認もIP変化の追従も一切せず
     同じ固定IPを5分おきに再送し続けていた検証残骸だった(本番の
     自己ホスト運用とは無関係、放置すればいずれそのIPが実際には
     存在しないPCを指し続けるだけの死んだDNSエントリになる)。
  2. **対応**: VPS(`conoha`)上の`duckdns-open-english.timer`/
     `.service`を停止・無効化・削除した(`duckdns-easy-web.timer`は
     共有デモ`easy-web.tokyo`用の別物のため無変更)。
  3. **設計の正直な再確認**: WEB版(`easy-web.tokyo`)・Windows版とも
     既に`open-easy-web`を土台としている——WEB版はVPS上の
     `open-easy-web`(nginx経由)配下、Windows版は同梱の
     `open-easy-web`ローカルモード(`OPEN_EASY_WEB_LOCAL_MODE=1`、
     既定`127.0.0.1:8090`)配下。どちらも`open-easy-web`の「簡単
     ドメイン設定ウィザード」(DuckDNS自動登録・最大20ドメイン対応、
     有料ドメインは購入済みのものをポート転送で指す案内)を個別に
     使い、**それぞれ別々の実ドメインを利用者自身が登録する**設計に
     なっている(open-english自身の`POST /v1/duckdns/update`も
     同型の機能を単体でも提供)。
  4. **未対応・正直な開示**: 「有料ドメインの簡単登録機能」は現状
     「購入済みドメインのDNSをポート転送で指す」案内にとどまり、
     レジストラAPI連携による**新規ドメインの自動購入**は
     `open-easy-web`・`open-english`のいずれにも実装されていない
     (対応するには特定レジストラのAPI選定・課金処理・利用規約確認が
     必要なため、ユーザーへの確認待ち)。
  - 次にすべきこと: 上記4番についてユーザーに要否を確認してから着手
    するかどうかを決める。ドキュメント上「`open-english.duckdns.org`」
    という具体例を今後案内する際は、共有デモの例ではなく利用者自身が
    新規取得する専用サブドメインである旨を明記する。
...
- **2026-08-28 ログイン方式を4択(パスワード無し/email OTP/QR撮影
  ...
  WEBカメラでも撮影してログインする二段階ログインシステムに統一」→
  「パスワード無し・email OTP・QR撮影のみ・email OTP+QR撮影と選べる
  ように」→「QR撮影はその時のパスワードを自動送受信・自動承認のみ」
  という段階的な指示への対応)**:
  1. **旧TOTP(認証アプリへ秘密鍵を事前登録し6桁コードを手入力)方式は
     廃止**。代わりに`server/src/auth.rs`へ「QR確認ログイン」を新設——
     ログインのたびに使い捨ての短命(3分・1回限り)なQRコード(確認用
     URLを埋め込むだけ)を生成し、スマホ/タブレット/WEBカメラ搭載端末で
     そのURLを開く(`qr-confirm.html`、新設)だけで**ボタン操作も無しに
     自動的に**ログインが確認される設計にした。
  2. **4つのログイン方式を`login_mode`設定(`none`/`otp`/`qr`/`otp_qr`)
     で選択可能に**: 1.パスワード無し、2.email/SMS OTP単体(従来の
     `verify_otp`をそのまま利用)、3.QR撮影のみ(事前のメール確認
     不要、`start_qr_only_login`)、4.email/SMS OTP(第一要素)+QR撮影
     (第二要素、真の2FA、`verify_otp_start_2fa`)。旧`login_required`
     真偽値は後方互換のため残し、`login_mode`未設定時は
     `true`→`"otp"`(2FA導入前の元の挙動)、`false`→`"none"`へ自動
     変換する。
  3. **正直な開示(誇張しないこと)**: QR確認は「その場でカメラ画像を
     解析する生体認証」ではなく、単に短命URLを開いてサーバーへPOSTする
     操作に過ぎない——画面を他人に見られた場合、その人もURLを開けて
     しまう(有効期限3分+1回限りの消費で被害範囲を抑える設計)。この
     限界を`qr-confirm.html`・ログインゲートUI双方に明記した。
  4. **実機検証(型チェック・ビルド成功だけで完了と報告しない方針の
     徹底)**: `cargo build --release`成功(警告なし)、`cargo test
     --release`54件全green(既存の非回帰込み)。実際にサーバーを起動し、
     (a) `POST /v1/auth/config`で4モードそれぞれへ切り替え・
     `GET /v1/auth/config`で反映を実HTTPで確認、(b) `qr`モードで
     `POST /v1/auth/qr-login/start`→`whoami`→`confirm`→`status`→
     `finish`の一連を実HTTPで確認しセッションCookieの発行まで確認、
     (c) 実ブラウザ(Claude Browser)で`qr`モードのログインゲートが
     メール欄を隠しQR撮影ボタンのみを表示すること、ボタン押下で実際に
     QRコードが表示されること、**別タブで実際に確認URLへ`navigate`し、
     ページ読み込みだけでボタン操作なしに自動承認が完了すること**
     (`qr-confirm.html`の実ページ動作、モックではない)、その後2秒
     ポーリングでプライマリタブ側のログインゲートが自動的に閉じる
     ことを確認した。検証後、テストサーバー・ブラウザタブは終了済み。
  5. `node --check app.js`成功。`android/app/src/main/assets/webroot/`
     へも`index.html`/`app.js`/`style.css`/`qr-confirm.html`を同期
     コピー済み。
  - 次にすべきこと: (1) `otp_qr`モード(email OTP+QR撮影の完全な2FA)の
    実SMTPを使ったE2E検証(この開発機にSMTPサーバーが無いため、コード
    検証成功後にQRが表示されるところまでは`qr`単体テストで代替確認)、
    (2) ユーザーからの依頼「open-englishに限らず」——他のログイン機能を
    持つリポジトリ(open-easy-web等)への同じ4択方式の横展開は今回未着手
    (対象範囲の棚卸しから次回着手)、(3) VPS(easy-web.tokyo)への
    デプロイ・本番反映。
...
- **2026-08-27(続き27) ログインゲートの表示バグ修正+携帯電話番号欄の
  ...
  1. **【実バグ修正】ログインモーダルの上端が見えなくなるバグ**:
     `.login-gate`(`position:fixed; inset:0; display:flex;
     align-items:center;`)は、中身(メール1・メール2・携帯電話・QR
     案内等)が増えて縦に長くなった結果、垂直中央寄せされたボックスの
     上端がビューポート外(y<0)へはみ出し、**スクロールする手段が
     どこにも無かった**ため上部が事実上見えなくなっていた
     (`overflow-y`が未設定)。`align-items: flex-start` +
     `overflow-y: auto`(外側の`.login-gate`)+`margin: auto`
     (内側の`.login-gate-box`、コンテンツが短い場合は引き続き中央
     寄せ、長い場合は上端から表示されスクロール可能)へ修正。
     **実機検証**: 実際にサーバーを起動し、Claude Browserで
     `getBoundingClientRect()`により修正前は起こり得た負のtop座標が
     解消されボックス上端が`y=24`(可視範囲内)に来ること、
     `gate.scrollTop = gate.scrollHeight`でボックス下端まで実際に
     スクロールでき、それまで到達できなかった要素が可視範囲に入る
     ことを確認した。
  2. **携帯電話番号欄をメール1・メール2と同じ並びへ移動**: 専用の
     メール入力欄(`login-totp-email`)を廃止し、メール1
     (`loginEmailEl`)の値をそのままQRコードの紐付け先として使う
     よう変更(利用者がメールアドレスを3回入力させられる手間を解消)。
     携帯電話番号欄(`login-totp-phone-label`)はメール2の直後へ移動し、
     「新設」を告げる派手な見出し(「📱 New (2026-08-27)」)は削除した
     (ユーザー指示「それが新設された事は、日本語も英語も表示しないで
     良いです」への対応)——ただし「SMSは送信されない」という機能上の
     正直な開示自体は、告知ではなく実際の挙動説明のため残した。
  3. **QRログイン欄の表示切替**: 携帯電話番号欄が空のうちは
     `#login-totp-section`(QR表示・認証アプリコード入力欄一式)を
     非表示にし、何か入力された時点で表示する設計へ変更(`input`
     イベントで`refreshLoginTotpSectionVisibility()`を呼ぶ)。
     **実機検証**: 空欄時は非表示、`"090-1234-5678"`を入力すると
     実際に表示されることをJS経由で確認した。
  4. **ログイン方式の整理(正直な開示)**: メール1・メール2(いずれも
     ワンタイムパスワード式)・携帯電話番号を紐付けたQR/認証アプリ
     コードの、**計3つの入口のうちどれか1つでログイン可能**という
     既存の設計(既に別セッションで実装済み)は変更していない——
     今回はUIの並び・表示条件・見出しの整理のみ。
  5. `node --check app.js`成功。`android/app/src/main/assets/webroot/`
     へも`app.js`/`index.html`/`style.css`を同期コピー済み。
  - 次にすべきこと: (1) 実際に`login_required:true`な状態
     (`GET /v1/auth/config`)での一気通貫の実機E2E検証(今回はJS経由で
     `login-gate`を強制表示させての検証にとどまる)、(2) ユーザーからの
     別の依頼「パスワードでログインする私のリポジトリの全てを修正して」
     は対象範囲(どのリポジトリを指すか、パスワード認証の何を修正
     したいのか——OTP/メール認証への置き換えか、パスワード自体の
     強化か等)が不明確なため、今回は着手せず次回ユーザーへ確認する。

- **2026-08-27(続き26) eガバメント議論トピックへ「オバマ元大統領の
  名言を具現化・実現化する手段」という一文を追加+14言語翻訳対応
  (ユーザー指示「以前バラク・オバマ前大統領が、政府とは大きいか
  小さいかではなく、機能するかしないかである。名言されたのを、
  具現化実現化する為にも、eガバメント、デジタルガバメントは、有効
  です。政策、国策として。回答にして。世界中の言語に翻訳して」への
  対応)**:
  1. **本文追加**: `egovOpinionText()`へ、冒頭で既に紹介している
     オバマ元大統領の発言(「政府とは大きいか小さいかではなく、機能
     するかしないかだ」)を締めくくりで再度参照し、「それを具現化・
     実現化するためにも、eガバメント・デジタルガバメントは有効な
     政策・国策だと考えます」という一文を日英併記で追加した。
  2. **14言語翻訳**: `EGOV_REALIZE_I18N`を新設し、前2エントリの
     `EGOV_TAX_I18N`/`EGOV_SMALLGOV_I18N`と同じ14言語へ翻訳。
     `egovSuffix()`は既存の`translation`/`translation2`ロジックへ
     `translation3`として同じパターンをもう一段追加しただけ(コード
     構造は変えず素直に積み重ねる設計を維持)。
  3. **実機検証**: 実際にサーバーを起動し、Claude Browserで
     (a) 既定(日英併記)で新しい一文が正しく含まれること、
     (b) イタリア語選択時に`EGOV_TAX_I18N`・`EGOV_SMALLGOV_I18N`・
     `EGOV_REALIZE_I18N`の**3つ全ての訳文**が正しく積み重なって
     追加されること、を確認した。`node --check app.js`成功(今回は
     Node.jsスクリプトでの機械挿入時のミスは発生せず、一発で構文
     エラー無し)。`android/app/src/main/assets/webroot/app.js`へも
     同期コピー済み。
  - 次にすべきこと: 残り約116言語への拡大(優先度の高い言語から
    段階的に、年金トピックと歩調を揃えるとよい)。このeガバメント
    トピックは今回で3段階の追記(税収不足対応/小さな政府への回答/
    オバマ発言の具現化)が積み重なったため、次回追記する場合は
    `egovOpinionText()`全体が冗長になっていないか一度見直すと良い。

- **2026-08-27(続き25) eガバメント議論トピックへ「小さな政府/税収不足
  対策への回答」という一文をさらに追加+14言語翻訳対応(ユーザー指示
  「小さな政府を目指したり、税収不足や歳入不足対策など、それを行なおう
  とする人々へ、公務員を大幅にリストラして、コールセンターを導入して、
  コンビニの端末やLINEアプリなどでそれを実現出来るので、回答になり
  えます。英語と日本語と世界中の言語に翻訳して追加して」への対応)**:
  1. **本文追加**: `egovOpinionText()`へさらに一文を日英併記で追加
     ——「小さな政府を目指す方や、税収不足・歳入不足対策を行おうと
     する方々への回答にもなり得ます。公務員を大幅にリストラし、
     コールセンターを導入し、コンビニの端末やLINEアプリなどを活用
     すれば実現可能だからです。これも政策として国策と言えるレベル
     だと考えます。」
  2. **14言語翻訳**: `EGOV_SMALLGOV_I18N`を新設し、前エントリの
     `EGOV_TAX_I18N`と同じ14言語へ今回追加した一文のみを翻訳した。
     `egovSuffix()`は既に取得済みの`code`/`baseCode`を再利用し、
     `EGOV_TAX_I18N`の訳文に続けて`EGOV_SMALLGOV_I18N`の訳文も
     追加するよう拡張(コードの重複を避け、2つ目の`if`ブロックとして
     素直に追加)。
  3. **【実機検証はしていないが記録すべき編集時の実ミス】オブジェクト
     リテラルの閉じ括弧を1つ書き忘れ→型チェックが実際にエラーを
     検出**: Node.jsスクリプトでの機械的な文字列挿入時、
     `EGOV_TAX_I18N`オブジェクトの閉じ括弧`};`を追加し忘れたまま
     新しい`EGOV_SMALLGOV_I18N`の中身を挿入してしまい、
     `node --check app.js`が`Unexpected identifier`エラーで実際に検出
     した。修正後も別の場所で閉じ括弧が重複するミスを重ねて発生させて
     しまったが、これも`node --check`で即座に検出・修正できた——
     「まず`node --check`を通す」という既存の徹底事項が、今回のような
     機械的なテキスト編集のミスをその場で捕まえる具体例となった。
  4. **実機検証**: 実際にサーバーを起動し、Claude Browserで
     (a) 既定(日英併記)で新しい一文が正しく含まれること、
     (b) フランス語選択時に`EGOV_TAX_I18N.fr`と
     `EGOV_SMALLGOV_I18N.fr`の**両方**が正しく追加されること、を確認
     した。`node --check app.js`成功。`android/app/src/main/assets/
     webroot/app.js`へも同期コピー済み。
  - 次にすべきこと: 残り約116言語への拡大(優先度の高い言語から
    段階的に、年金トピックと歩調を揃えるとよい)。

- **2026-08-27(続き24) eガバメント議論トピックへ「税収不足・歳入不足への
  国策レベルの対応策」という一文を追加+14言語翻訳対応(ユーザー指示
  「eガバメントやデジタル・ガバメントの導入は、政府や地方自治体や
  都道府県庁や州などで、税収不足や歳入不足などにも対応できる政策として
  国策と言えます。英語と日本語と世界中の言語に翻訳して追加して」への
  対応)**:
  1. **本文追加**: 既存`egovOpinionText()`(オバマ元大統領の発言紹介+
     eガバメント推進の一意見、`egovSuffix`経由で"eガバメント"等の
     話題検出時に表示)の末尾へ、ユーザー指示の趣旨をそのまま反映した
     一文を日英併記で追加した——「政府・地方自治体・都道府県庁・
     (海外の)州などにおいて、税収不足や歳入不足にも対応できる政策
     として、国策と言えるレベルの重要施策」。
  2. **14言語翻訳**: `EGOV_TAX_I18N`を新設し、年金トピック
     (`PENSION_PROPOSAL_I18N`)と全く同じ14言語(西・仏・独・葡・露・
     中(簡体)・韓・ヒンディー・アラビア・ヘブライ・ペルシャ・
     ウクライナ・伊・中(繁体/台湾語))へ、追加した一文のみを翻訳した
     (本文全体〈オバマ発言紹介部分〉は日英併記のみのまま、今回追加
     した一文だけを多言語化——過度に肥大化させないための意図的な
     スコープ限定)。`egovSuffix()`は`preferredWorldLangCode()`
     (年金トピックと共通のヘルパー、完全一致優先→基底コードへ
     フォールバックする設計)を再利用し、選択言語の訳文があれば末尾へ
     追加する。
  3. **実機検証**: 実際にサーバーを起動し、Claude Browserで
     (a) 既定(日英併記)で新しい一文が正しく含まれること、
     (b) ドイツ語選択時にドイツ語訳が末尾へ追加されること、
     (c) 既存のオバマ発言紹介・「一意見」の開示文が引き続き正しく
     表示されること(非回帰)、を確認した。`node --check app.js`
     成功。`android/app/src/main/assets/webroot/app.js`へも同期
     コピー済み。
  - 次にすべきこと: 残り約116言語への拡大(優先度の高い言語から
    段階的に、年金トピックと歩調を揃えるとよい)。

- **2026-08-27(続き23) 年金トピックの翻訳対応言語を6言語追加(計14言語、
  ユーザー指示「アラビア語、ヘブライ語、イラン(ペルシャ語)、
  ウクライナ語、イタリア語、台湾語も追加して」への対応)+実機検証で
  発見した実バグを修正**:
  1. **追加言語**: `PENSION_PROPOSAL_I18N`へアラビア語(`ar`)・
     ヘブライ語(`he`)・ペルシャ語(`fa`)・ウクライナ語(`uk`)・
     イタリア語(`it`)・台湾語/繁体字中国語(`zh-Hant`)の6言語を追加し、
     計14言語対応となった(前エントリの8言語+今回の6言語)。
  2. **【実機検証で発見・修正した実バグ】`zh-Hant`が常に`zh`(簡体字)の
     訳文で上書きされていた**: `preferredWorldLangCode()`が
     "zh-Hant"→"zh"のように基底コードへ丸めてから返す設計だったため、
     `pensionSuffix()`が`PENSION_PROPOSAL_I18N["zh-Hant"]`を一度も参照
     できず、台湾語を選んでも簡体字の訳文が表示されてしまっていた
     (型チェック・構文チェックでは検出できず、実際に`world:zh-Hant`を
     選択してJS経由で呼び出して初めて発覚)。修正: 完全一致
     ("zh-Hant")を先に試し、無ければ基底コード("zh")へフォールバック
     する順序へ変更(`preferredWorldLangCode()`からは丸め処理自体を
     削除し、`pensionSuffix()`側で「完全一致優先→基底コードへ
     フォールバック」のロジックを持たせた)。
  3. **実機検証**: 実際にサーバーを起動し、Claude Browserで
     (a) 台湾語(`world:zh-Hant`)選択時に実際に`zh-Hant`用の訳文
     (繁体字)が返ること(修正前は`zh`用の簡体字訳文が返っていたことを
     確認した上で修正・再検証)、(b) ヘブライ語選択時に正しい訳文が
     返ること、(c) 既存の韓国語(基底コードのみ、ハイフン無し)が
     引き続き正しく動作すること(非回帰確認)、を確認した。
     `node --check app.js`成功。`android/app/src/main/assets/webroot/
     app.js`へも同期コピー済み。
  - 次にすべきこと: 残り約116言語への拡大(優先度の高い言語から
    段階的に)。

- **2026-08-27(続き22) 年金トピックの意見文を「利用者が選択した世界の
  言語」への翻訳にも対応(ユーザー指示「上記回答は、日本語と英語で
  回答してその他言語を選択していれば翻訳して」→「利用者が選択した
  世界の言語への翻訳も対応して」への対応)**:
  1. **設計**: `pensionSuffix()`は常に日英併記を返す(既存動作を変更
     しない)が、`preferredWorldLangCode()`(学びたい言語`world:<code>`
     または母国語設定から言語コードを取得する汎用ヘルパー、既存の
     `quizPreferredLangCode()`から`currentQuizTexts`依存を除いて
     切り出したもの)が返すコードが`PENSION_PROPOSAL_I18N`に存在すれば、
     その言語への訳文を日英併記の前に追加する。日本語・英語が選択
     されている場合は追加訳文を出さない(既に日英併記があるため)。
  2. **対応言語(8言語、正直な開示)**: データサイエンティスト擬似模擬
     TEST(前々エントリ)で既に用意していた主要8言語(スペイン語・
     フランス語・ドイツ語・ポルトガル語・ロシア語・中国語・韓国語・
     ヒンディー語)から着手した。全世界の言語を一度に翻訳することは
     現実的でないため、未対応言語では従来通り日英併記のみ返す
     (「対応済み」を偽らない)。訳文はネイティブレビュー未実施の
     最善努力の翻訳。
  3. **実機検証**: 実際にサーバーを起動し、Claude Browserで
     (a) 学びたい言語を韓国語(`world:ko`)に設定した状態で
     `pensionSuffix()`を呼ぶと、韓国語訳+既存の日英併記の両方が
     含まれること、(b) 学びたい言語を英語のままにした場合は韓国語訳が
     追加されず従来通り日英併記のみになること、を確認した。
     `node --check app.js`成功。`android/app/src/main/assets/webroot/
     app.js`へも同期コピー済み。
  - 次にすべきこと: (1) 残り約120言語への拡大(優先度の高い言語から
    段階的に)、(2) 同じ仕組み(`preferredWorldLangCode`+
    `*_I18N`辞書)を他の意見系トピック(消費税提案・年収の壁提案等)へも
    展開するかの検討。

- **2026-08-27(続き21) 老後資金・年金問題への意見トピック(`pensionSuffix`)を
  新設(ユーザー指示「2,000万円問題、麻生太郎が年金を5万円もらって生活しろ、
  年金や貯金がゼロ/ほとんど無い方は老後の生活をどうしたら良いか、といった
  質問があれば、まずは結論から…」+追記指示「基礎年金+厚生年金+増額年金を
  専業主婦や厚生年金に入っていない方も含めて全員参加型の皆保険が可能」への
  対応)**:
  1. **既存パターンの踏襲**: `consumptionTaxSuffix`/`fairTradeSuffix`と
     全く同じ設計——AI推論(GPT-2)を経由させず、開発者(ユーザー)個人の
     具体的な政策的意見を「【老後資金・年金問題への提案(開発者からの
     一意見)】」という明記付きの固定文で返す。日次利用回数は消費しない。
  2. **判定ロジック(`isPensionQuestion`)**: 「2000万円問題」「麻生太郎」
     「年金5万円」等の**話題語自体が既に具体的な問題提起になっている
     強いキーワード**は単独で反応してよいとし、「老後資金」「老後の生活」
     等の一般的な語は疑問詞("どうしたら"等)とのAND条件を維持して誤検出を
     避けた(既存の消費税判定より一段緩めた判定基準——「2000万円問題」と
     言った時点でほぼ確実にこの話題についての発話であるため)。単体テスト
     (Node.jsで判定ロジックのみ抽出して検証)で6ケース全てPASS——3つの
     肯定例(2000万円問題/麻生太郎+年金5万円/年金・貯金がほぼ無い方の
     老後相談)と3つの否定例("今日の天気"・英語の無関係文・「年金制度に
     ついて解説してください」のような一般的な言及のみで疑問詞を伴わない
     文)を確認。
  3. **本文の内容**: 財源はeガバメント/デジタルガバメント化(コンビニの
     マルチコピー機・情報端末・LINEアプリ等での行政手続き対応)+在宅勤務の
     コールセンター/TVチャットサービス導入により、市区町村・都道府県庁の
     公務員を必要最低限までリストラして確保可能、という提案を「結論から」
     の形で提示し、続けて年金運用益が国民へ還元されてこなかった問題への
     言及、そして追記指示に基づき基礎年金+厚生年金+増額年金の全員参加型
     皆保険化(専業主婦(夫)・厚生年金未加入者を含む)を提案する一文を
     追加した。日英併記(既存の意見系トピックと同じ形式)。
  4. **実機検証**: `node --check app.js`成功。実際にサーバー+
     `aruaru-llm`(distilgpt2)を起動し、Claude Browserで実際に
     「老後2,000万円問題についてどう思いますか？年金や貯金がほとんど
     無い方はどうしたら良いのですか？」を送信し、AI応答(GPT-2の生成
     本文、内容自体は無関係だがそれは既存の制約通り)の直後に、
     日英併記の意見文(追記した皆保険化の一文を含む)が正しく追記される
     ことを確認した。`android/app/src/main/assets/webroot/app.js`へも
     同期コピー済み。
  - 次にすべきこと: 特になし(今回のスコープは完了)。今後さらに追記
    指示があれば`pensionProposalText()`の該当箇所へ追記すればよい。
...
- **2026-08-27(続き18) 図面操作パネルの「未検証」だった成功パスを実機で
  ...
  1. **実機検証(前回エントリで「未検証」としていた項目)**: 実際に
     `aruaru-llm.exe`(distilgpt2、既定ポート4600)+`open-cg-cad-server.exe`
     +`open-english-server.exe`の3プロセスを同時起動し、Claude Browserで
     実際にopen-englishの図面操作パネルから**複数図面の合成**を実行
     ——open-english→open-cg-cad→aruaru-llmという3段の経路全体が実際に
     成功し、実際のGPT-2生成テキストが「✅ Saved as drawing #12」として
     画面に表示されることを確認した(モックではない、実際のトークン
     生成に約15〜17秒)。**「エラー経路の確認までに留めた」という前回の
     開示を解消**。
  2. **発見した使いやすさの穴と修正**: 上記検証中、パネルが
     aruaru-llmの接続先を常に既定値(`http://localhost:4600`)固定で
     open-cg-cadへ送っており、**利用者が別ポート/別ホストでaruaru-llmを
     動かしている場合に対応できない**ことに気付いた(既存のworld-lab
     パネル等は接続先を調整できる設計になっているのに、この新パネルは
     一律固定だった、という不整合)。`#cg-cad-aruaru-llm-base`入力欄
     (空欄なら既定値のまま)を新設し、`app.js`の`cgCadAruaruLlmBaseOverride()`
     で合成・再設計の両リクエストへ`aruaru_llm_base`として渡すよう修正。
     **実機再検証**: 3プロセスを非既定ポート(aruaru-llm:18750、
     open-cg-cad:18751、open-english:18752)で起動し、この上書き欄に
     `http://127.0.0.1:18750`を入力した状態で実際に再設計を実行→
     正しく18750番ポートのaruaru-llmへ到達し実生成が成功することを確認
     (上書きが効いていなければ既定の4600番へ送られ失敗するはずの
     ケースを、実際に非既定ポート構成で再現して確認した)。
  - 次にすべきこと: (1) 実VPS/実GitHubトークンでのGitHub書き込み
    (commit)E2E検証(引き続き未実施、実認証情報が必要)、(2) 実VPSへの
    SSH接続E2E検証(同様に未実施)、(3) 本番(easy-web.tokyo)環境での
    このパネルの実地確認(現状はaruaru-llmがVPS上で稼働していないため、
    本番では常に「could not reach aruaru-llm」の正直なエラーになる—
    —これはアーキテクチャ上の意図通り、利用者が自分のPCでaruaru-llmを
    起動する前提のため異常ではない)。
...
既定方針(正本は`open-raid-z/CLAUDE.md`同日エントリ)

本セッションでopen-english経由でaruaru-db側へ「REST APIを不要にして、
APIキーを自動発行・自動承認・自動破棄・自動削除で自動管理して」との
指示を出したことをきっかけに、`aruaru-db`が(1)Raft/WALのノード間
プロトコルをREST/JSON-over-HTTPから生TCPバイナリフレームへ完全移行
(RPoemの`KeyGuardian`設計を参考にしたAPIキー自動ライフサイクル管理も
実装)、(2)GraphQL管理APIの固定値スタブを実データ接続へ修正、を行った
(詳細は`aruaru-db/CLAUDE.md`2026-08-29エントリ参照)。日英Web検索
(Shopify/資産運用サービス「マネイロ」の実例、【2026-08-29訂正】従来
「JX通信社」と誤記していたが実際は無関係のため訂正)に基づき、
REST APIの即時全廃は2026年時点の
実務標準ではないと判断し、エコシステム全体の方針として
`open-raid-z/CLAUDE.md`へ「(a)ノード間の低レベルプロトコルはバイナリ化、
(b)人間向け管理APIはREST/GraphQL併存のまま段階的に移行」という2軸を
記録した。

**open-english自身への適用状況(正直な開示)**: 本リポジトリの
`server/`(open-english-server)は静的ファイル配信+`/v1/db/*`等の
薄いREST APIのみで、Raftのようなノード間合意プロトコルは持たない
ため、(a)の対象箇所は無い。(b)については`server/`のREST API群を
GraphQLへ移行する具体的な計画は今回未着手。
...
## HANDOFF 2026-09-21: 作者のオリジナル問題の巡回・APK未添付バグ修正(v0.8.10)

- **クイズ**: ユーザー報告「その2だけ表示されて、その1とその3が表示されない」。3問とも出題自体は
  できていたが、選び方が「直前と違う問題をランダム」で、初回は必ず「その1」以外・偶然の偏りがあった。
  → **「その1→その2→その3」の順に必ず巡回**(位置は`localStorage`の`open-english.quizNextIndex`)。
  「その2」「2問目」「問題3」等の**番号指定**、「全部」「1、2、3」「１２３の問題」で**3問を続けて表示**
  (`quizNumberRequest`)。出題は常に日本語+英語の併記。第1問の見出しにも「その1」を付けた。
- **v0.8.9でAPKがReleaseに付かなかった原因**: `open-english-pc`再統合時に`release.yml`のパス
  `client/mobile/android`→`mobile/android`へ直した際、APK集約先`../../../android-apks`の階層を
  直し忘れ(リポジトリ外を指していた)。`../../android-apks`へ修正。
...
**追記2(2026-09-21)**: 同日2回目以降は、バナー無しで**ニュースの鮮度だけ確認**する
(`GET {aruaru-llm}/v1/news/latest`の`fetched_at_unix`)。未収集または6時間より古い場合だけ
`POST /v1/news/refresh`を呼び、新しければ収集しない。確認・収集の試行は1時間に1回まで
(`open-english.newsCheckAt`、失敗が続いても連打しない)。1日の最初の読み込みは従来通り無条件で収集。
注意: 公開サイトの`apiBase`は各訪問者自身の`localhost:4600`(自分のaruaru-llm)を指す設計のため、
訪問者側にaruaru-llmが無ければ確認は静かに失敗して何もしない(仕様)。

**追記3(2026-09-21)**: 「問題１の答えを教えて」で答えが出ないバグを修正。原因は「問題」+「教えて」を
含むため**出題要求(`isQuizRequest`)と誤判定**されていたこと。`askTrainer`で「答え/解答/正解/answer/
solution」を含む要求を**出題より先に答えの要求として処理**(番号指定=その問題の答え、全部=3問すべて、
番号無し+出題済み=直前の問題の答え)。`quizNumberRequest`は英語(`puzzle 2`/`problem 1`/`the third puzzle`/
`quiz no.3`)にも対応。⚠️過去の教訓: Nodeの`-e`をbashのダブルクォートで書くと`\b`が**バックスペース制御文字**
