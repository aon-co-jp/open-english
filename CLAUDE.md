...
- **2026-08-23 (続き5) クイズを3問へ拡張+「学年別・家庭教師コース」を新設
  ...
  5. **落ちこぼれ防止(スマイルゼミ的な適応フォロー、ユーザー追加要望)**:
     各問題は任意で`easier`(同じ単元・同じ考え方のまま1段階易しくした
     類題)を持つ。採点で間違え、かつ`easier`があれば
     「🌱 もう少し易しい問題に挑戦」ボタンが現れ、その類題を出題する。
     **正直な開示**: (a) **AI生成ではなく人手で書いた静的なペア**、
     (b) **1段階のみ**で多段階(さらに易しく)は未実装、(c) 全問には
     用意しておらず(現状は各教科の代表的な2〜5問)、易しい類題が無い
     場合は「まだ用意できていません(準備中です)」と正直に表示する。
